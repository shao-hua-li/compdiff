...
static u32 flag_drop_ratio = 0;
static s32 fsrv_ctl_fd_compiler[MAX_NUM_COM],
           fsrv_st_fd_compiler[MAX_NUM_COM],
           forksrv_pid_compiler[MAX_NUM_COM],
           dev_stdout_fd[MAX_NUM_COM],
...
EXP_ST u64  total_diff_compiler;
...
EXP_ST void setup_shm(void) {
  ...
  memset(virgin_diff, 255, MAP_SIZE);
...
EXP_ST void init_forkserver_compiler(char** argv, u32 idx_compiler) {
  ...
  ACTF("Compiler--Spinning up the fork server...FORKSRV_FD=%d", forksrv_fd_compiler);

  if (pipe(st_pipe) || pipe(ctl_pipe)) PFATAL("Compiler--pipe() failed");

  forksrv_pid_compiler[idx_compiler] = fork();

  if (forksrv_pid_compiler[idx_compiler] < 0) PFATAL("Compiler--fork() failed");

  if (!forksrv_pid_compiler[idx_compiler]) {
    ...
    if (!getrlimit(RLIMIT_NOFILE, &r) && r.rlim_cur < forksrv_fd_compiler + 2) {

      r.rlim_cur = forksrv_fd_compiler + 2;
      setrlimit(RLIMIT_NOFILE, &r); /* Ignore errors */
    ...
    dup2(dev_stdout_fd[idx_compiler], 1);
    dup2(dev_stderr_fd[idx_compiler], 2);
    ...
    if (dup2(ctl_pipe[0], forksrv_fd_compiler) < 0) PFATAL("dup2() failed");
    if (dup2(st_pipe[1], forksrv_fd_compiler + 1) < 0) PFATAL("dup2() failed");
    ...
    close(dev_urandom_fd);
    close(fileno(plot_file));
    close(dev_stdout_fd[idx_compiler]);
    close(dev_stderr_fd[idx_compiler]);
    ...
    setenv("MSAN_OPTIONS", "exit_code=" STRINGIFY(MSAN_ERROR) ":"
                           ...
                           "allocator_may_return_null=1:"
                           "msan_track_origins=0", 0);
    u8 curr_forksrv_fd[10];
    sprintf(curr_forksrv_fd, "%d", forksrv_fd_compiler);
    setenv("DIFF_FORKSRV", curr_forksrv_fd, 0);
    argv[0] = target_path_compiler[idx_compiler];

    execv(target_path_compiler[idx_compiler], argv);
  ...
  forksrv_fd_compiler = forksrv_fd_compiler + 4;
  ...
  fsrv_ctl_fd_compiler[idx_compiler] = ctl_pipe[1];
  fsrv_st_fd_compiler[idx_compiler]  = st_pipe[0];
  ...
  rlen = read(fsrv_st_fd_compiler[idx_compiler], &status, 4);
  ...
  if (rlen == 4) {
    OKF("Compiler--All right - fork server is up.");
    return;
  }

  if (child_timed_out)
    FATAL("Compiler--Timeout while initializing fork server (adjusting -t may help)");
...
static u8 run_target_compiler(char** argv, u32 timeout, s32 idx_compiler) {
  ...
  if ((res = write(fsrv_ctl_fd_compiler[idx_compiler], &prev_timed_out, 4)) != 4) {

    if (stop_soon) return 0;
    RPFATAL(res, "Compiler--Unable to request new process from fork server (OOM?)");

  }

  if ((res = read(fsrv_st_fd_compiler[idx_compiler], &child_pid, 4)) != 4) {
  ...
  if (child_pid <= 0) FATAL("Compiler--Fork server is misbehaving (OOM?)");
  ...
  if ((res = read(fsrv_st_fd_compiler[idx_compiler], &status, 4)) != 4) {
...
static u8 differential_compilers(char** argv, u32 timeout, void* mem, u32 len) {
  ...
  if (flag_use_output) {
    for (int idx_com=0; idx_com < NUM_COM; idx_com++) {
      dev_stdout_fd[idx_com] = dev_output_fd;
    }
  }
  ...
  for (int idx_com=0; idx_com < NUM_COM; idx_com++) {
    ...
    if (ftruncate(dev_stdout_fd[idx_com], 0)) PFATAL("ftruncate() failed");
    if (ftruncate(dev_stderr_fd[idx_com], 0)) PFATAL("ftruncate() failed");
    lseek(dev_stdout_fd[idx_com], 0, SEEK_SET);
    lseek(dev_stderr_fd[idx_com], 0, SEEK_SET);
  }
  ...
  while ((bytes = read(dev_stdout_fd[0], data, 1024)) != 0) {
    MD5_Update(&first_mdContext, data, bytes);
  }
  while ((bytes = read(dev_stderr_fd[0], data, 1024)) != 0) {
    MD5_Update(&first_mdContext, data, bytes);
  }
  ...
  for (int idx_com=1; idx_com < NUM_COM; idx_com++) {
    ...
    MD5_CTX mdContext;
    MD5_Init (&mdContext);
    while ((bytes = read(dev_stdout_fd[idx_com], data, 1024)) != 0){
      MD5_Update(&mdContext, data, bytes);
    }
    while ((bytes = read(dev_stderr_fd[idx_com], data, 1024)) != 0){
      MD5_Update(&mdContext, data, bytes);
    }
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fprintf(plot_file, 
          "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %llu, %u, %0.02f\n",
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s/diffs", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);
...
#define SP20    SP10 SP10
  ...
  sprintf(tmp, "%s (%s unique)", DI(total_diff_compiler), DI(unique_diff_compiler));

  SAYF (bSTG bV bSTOP "  total diffs : " cRST "%-22s " bSTG bV "\n", tmp);
...
static u8 fuzz_one(char** argv) {
  ...
  if (not_on_tty) {
    ACTF("Fuzzing test case #%u (%u total, %llu uniq diffs found, %llu uniq crashes found)...",
         current_entry, queued_paths, unique_diff_compiler, unique_crashes);
    fflush(stdout);
  }
...
static void handle_stop_sig(int sig) {
  ...
  for (int idx_com=0; idx_com < NUM_COM; idx_com++) {
    if (forksrv_pid_compiler[idx_com] > 0) kill(forksrv_pid_compiler[idx_com], SIGKILL);
  }
...
EXP_ST void check_binary(u8* fname) {
  ...
  if (strchr(fname, '/') || !(env_path = getenv("PATH"))) {
    ...
    while (env_path) {
      ...
      for (int idx_com=0; idx_com < NUM_COM; idx_com++) {
        ck_free(target_path_compiler[idx_com]);
        target_path_compiler[idx_com] = 0;
      }
    }

    if (!target_path) FATAL("Program '%s' not found or not executable", fname);
    for (int idx_com=0; idx_com < NUM_COM; idx_com++) {
        if (!target_path_compiler[idx_com]) FATAL("Program '%s_%d' not found or not executable", fname, idx_com);
    }
...
EXP_ST void setup_stdout_file_compiler(void) {

  if (flag_use_output) {
    OKF("Using output...%s", output_file);
    u8* fn = alloc_printf("%s/%s", out_dir, output_file);
    unlink(fn); /* Ignore errors */

    dev_output_fd = open(fn, O_RDWR | O_CREAT | O_EXCL, 0600);

    if (dev_output_fd < 0) PFATAL("Unable to create '%s'", fn);

    ck_free(fn);
  }

  for (int idx_com=0; idx_com < NUM_COM; idx_com++) {
    u8* fn = alloc_printf("%s/.cur_output_%d", out_dir, idx_com);

    unlink(fn); /* Ignore errors */

    dev_stdout_fd[idx_com] = open(fn, O_RDWR | O_CREAT | O_EXCL, 0600);

    if (dev_stdout_fd[idx_com] < 0) PFATAL("Unable to create '%s'", fn);

    ck_free(fn);
  }

  for (int idx_com=0; idx_com < NUM_COM; idx_com++) {
    u8* fn = alloc_printf("%s/.cur_error_%d", out_dir, idx_com);

    unlink(fn); /* Ignore errors */

    dev_stderr_fd[idx_com] = open(fn, O_RDWR | O_CREAT | O_EXCL, 0600);

    if (dev_stderr_fd[idx_com] < 0) PFATAL("Unable to create '%s'", fn);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+u:p:r:c:i:o:f:m:b:t:T:dnCB:S:M:x:Q")) > 0)

    switch (opt) {

      case 'u':
      if (sscanf(optarg, "%u", &flag_use_output) < 1 ||
              optarg[0] == '-') FATAL("Bad syntax used for -u");
        flag_use_output = 1;
        break;
      ...
      case 'r':
      if (sscanf(optarg, "%u", &flag_drop_ratio) < 1 ||
              optarg[0] == '-') FATAL("Bad syntax used for -u");
        break;
